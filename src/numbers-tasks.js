...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  if (magnitudeA === 0 || magnitudeB === 0) {
    throw new Error('One of the vectors is zero, angle is undefined.');
  }
...
function roundToPowerOfTen(num, pow) {
  if (pow < 0 || !Number.isInteger(pow)) {
    throw new Error('Power of 10 must be a non-negative integer.');
  }
 ...
 * @param {any} def - The default value to return if conversion fails.
 * @return {number} - The converted number or the default value.
 *
 * @example
 *   toNumber(null, 0) => 0
 *   toNumber('test', 0) => 0
 ...
 * 0.1, 0.2, 0.3 => 0.6
 */
function getSumOfNumbers(x1, x2, x3, decimalPlaces = 2) {
  if (
    typeof x1 !== 'number' ||
    typeof x2 !== 'number' ||
    typeof x3 !== 'number'
...
function getHypotenuse(a, b) {
  if (typeof a !== 'number' || typeof b !== 'number' || a < 0 || b < 0) {
    throw new TypeError('Both inputs must be valid non-negative numbers.');
  }
